...
namespace CRC_Master
{
    public class CRCMaster
    {
        ...
        {
            var CRC = 0;
            for (UInt32 pos = 0; pos < data.Length; pos++)
            {
                CRC = CRC + data[pos];
                CRC &= 0xff;
            }
        ...
        public static ushort CRC16Sick(byte[] data)
        {
            var crc = 0;
            var c = 0;
        ...
        public static ushort CRCXModem(byte[] data)
        {
            var crc = 0;
        ...
        public static ushort CRCFFFF(byte[] data)
        {
            var crc = 0xFFFF;
            for (var c = 0; c < data.Length; c++)
            {
        ...
        public static ushort CRC1D0F(byte[] data)
        {
            var crc = 0x1d0f;
            for (var c = 0; c < data.Length; c++)
            {
        ...
        public static ushort CRCKermit(byte[] data)
        {
            var crc = 0;
            for (var c = 0; c < data.Length; c++)
            {
        ...
        public static ushort CRCDNP(byte[] data)
        {
            ushort[] CRCTableDNP = new ushort[256];
            ...
            for (i1 = 0; i1 < 256; i1++)
            {
                ...
                for (j1 = 0; j1 < 8; j1++)
                {
                    if (((crc1 ^ c1) & 0x0001)!=0) crc1 = (crc1 >> 1) ^ 0xA6BC;
                    else crc1 = crc1 >> 1;
                    c1 = c1 >> 1;
                }
                CRCTableDNP[i1] =(ushort)crc1;
            }
        ...
        public static UInt32 CRC32(byte[] data)
        {
            ...
            UInt32[] CRCTable = new UInt32[] { 0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535,
            ...
            for (var i = 0; i < data.Length; i++)
            {
                r = (r >> 8) ^ CRCTable[data[i] ^ (r & 0x000000FF)];
            }
            return ~r;
        }
